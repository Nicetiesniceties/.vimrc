...
syn keyword undoneStatus FAIL NOTICE TODO UNDONE FALLBACK
syn keyword doneStatus DONE SCORE SKIP REMIND
...
hi def link doneStatus String "less blue
"hi def link noteNumber Number" purple
hi def link noteOperator cOperator "Typedef "yellow
hi def link noteType Typedef"yellow 
hi def link noteMonths Identifier "blue
hi def link noteDays Preproc "green
...
syntax region noteHighlight1 start=/\v˙/ skip=/\v\\./ end=/\v˙/ "option + h
syntax region noteHighlight2 start=/\v_/ skip=/\v\\./ end=/\v_/ 
syntax region noteUnderline start=/\v[¨]/ skip=/\v\\./ end=/\v[¨]/ "option + u
...
hi noteUnderline gui=underline cterm=underline

" personal settings
" -------------------------------------------------------------------
syn keyword Courses ADA 自動機 前瞻科技導論 LinearAlgebra SystemPrograming 
syn keyword Courses AmazingUniverse FinancialAccounting 
syn keyword ActivityType GIS Basketball Tutoring Personal SaxophoneQuartet Workout CSIE
hi def link ActivityType Identifier
hi Courses cterm=underline gui=underline

" trivia
hi def link Folded CursorLine
